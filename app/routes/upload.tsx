...
export const upload = () => {
  ...
  const handleAnalyze =async ({companyName,jobTitle,jobDescription,file}: {companyName: string,jobTitle: string,jobDescription: string,file: File}) => {
    ...
    if(!feedback) return setStatusText('Error: Failed to analyze resume');
     ...
     await kv.set(`resume:${uuid}`, JSON.stringify(data));
     setStatusText('Resume analyzed successfully, redirecting to review page...');
     console.log(data);
    ...
    <main className="bg-[url('/images/bg-main.svg')] bg-cover">
      ...
      <section className="main-section">
        <div className="page-heading py-16" >
          <h1>Calculated review for your dream job</h1>
          {isProcessing ? (
            <>
              <h2>{statusText}</h2>
            ...
            <h2>Drop your resume for the ATS Score</h2>
          )}
          {!isProcessing && (
            <form id={"upload-form"} onSubmit={handleSubmit} className={"flex flex-col gap-4"}>
              ...
              <button className={"primary-button"} type="submit">Upload Resume</button>
